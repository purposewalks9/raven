    ...
    function resolveModulePath(workspaceRoot2, moduleName, nodePath, tracer) {
      if (nodePath) {
        if (!path.isAbsolute(nodePath)) {
          nodePath = path.join(workspaceRoot2, nodePath);
        }
...
// ../../compiler/dist/chunk-SVI6HOOH.js
...
function diffShapes(previous, next) {
...
function formatShapeDiff(entries) {
...
var TypeChecker = class {
  ...
  currentReturnType;
  inferredReturnTypes;
  externalModelBindings = /* @__PURE__ */ new Map();
  constructor(options = {}) {
  ...
  check(program) {
    ...
    this.externalModelBindings = /* @__PURE__ */ new Map();
...
var WorkspaceRegistry = class {
  ...
  publish(name, type, external, file, location) {
...
function findRavenFiles(root) {
  const results = [];
  const walk = (dir) => {
    for (const entry of (0, import_fs.readdirSync)(dir)) {
      if (entry === "node_modules" || entry.startsWith(".")) continue;
      const full = (0, import_path.join)(dir, entry);
      const stats = (0, import_fs.statSync)(full);
      if (stats.isDirectory()) {
        walk(full);
      } else if ((0, import_path.extname)(full) === ".rv") {
        results.push(full);
      }
    }
  };
...
function resolveImportPath(fromFile, source, knownFiles) {
  if (!source.startsWith(".") && !(0, import_path.isAbsolute)(source)) return void 0;
  const base = (0, import_path.resolve)((0, import_path.dirname)(fromFile), source);
  const candidates = [base, `${base}.rv`, (0, import_path.join)(base, "index.rv")];
  return candidates.find((candidate) => knownFiles.has(candidate));
}
function buildProject(root) {
  const rootPath = (0, import_path.resolve)(root);
  const isDir = (0, import_fs.statSync)(rootPath).isDirectory();
  const filePaths = isDir ? findRavenFiles(rootPath) : [rootPath];
  const files = [];
  const diagnostics = [];
  for (const path of filePaths) {
    const source = (0, import_fs.readFileSync)(path, "utf8");
    try {
  ...
  const exportedFunctions = /* @__PURE__ */ new Map();
  ...
  const importedFunctionsByFile = /* @__PURE__ */ new Map();
  for (const file of files) {
    ...
    for (const stmt of file.ast.body) {
      if (stmt.type === "ImportDeclaration") {
        ...
        const sourceSignatures = resolvedPath ? exportedFunctions.get(resolvedPath) : void 0;
...
function toFsPath(uri) {
  return uri.startsWith("file://") ? (0, import_node_url.fileURLToPath)(uri) : uri;
}
...
function formatType2(type) {
  ...
  if (type.kind === "array") return `${formatType2(type.elementType)}[]`;
  if (type.kind === "record") {
    const fields = Object.entries(type.fields).map(([key, value]) => `${key}: ${formatType2(value)}`).join("; ");
    return `{ ${fields} }`;
  }
  return "unknown";
}
...
connection.onInitialize((params) => {
  ...
  if (rootUri) setWorkspaceRoot((0, import_node_url2.fileURLToPath)(rootUri));
  return {
    capabilities: {
