...
export class Parser {
  ...
  // Inside UI blocks, prop/child names can collide with reserved keywords
  // (e.g. `type email` in an `input` block, where `type` is also the
  // top-level `type` declaration keyword). This treats Keyword tokens as
  // identifier-like in that specific position only — expression parsing
  // elsewhere stays strict about keywords.
